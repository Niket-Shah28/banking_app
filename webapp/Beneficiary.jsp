...
<button class="btn btn-primary mb-3"
  ...
  <div class="card-box" role="region" aria-label="Add Beneficiary">
    ...
    <form action="AddBeneficiaryController" method="post">
      <div class="form-group">
        ...
        <input id="beneficiaryName" name="beneficiaryName" type="text" class="form-control" required>
      </div>

      <div class="form-group">
        <label for="beneficiaryAccountNumber" class="form-label">Beneficiary Account Number</label>
        <input id="beneficiaryAccountNumber" name="beneficiaryAccountNumber" type="text" class="form-control" required>
      </div>

      <div class="form-group">
        <label for="beneficiaryBankName" class="form-label">Beneficiary Bank Name</label>
        <input id="beneficiaryBankName" name="beneficiaryBankName" type="text" class="form-control" required>
      </div>
  ...
  <div class="table-box" role="region" aria-label="Existing Beneficiaries">
    <div class="card-header">EXISTING BENEFICIARIES</div>

    <c:choose>
      <c:when test="${empty beneficiaries}">
        <div class="no-data">No Beneficiaries Added Yet</div>
      </c:when>
      ...
      <c:otherwise>
        <div class="table-responsive">
          <table class="table w-100">
            <thead>
              <tr>
                <th class="text-start">Beneficiary Name</th>
            ...
            <tbody>
              <c:forEach var="b" items="${beneficiaries}">
                <tr>
                  <td class="text-start">${b.beneficiaryName}</td>
                  <td>${b.beneficiaryAccountNumber}</td>
                  <td>${b.beneficiaryBankName}</td>
