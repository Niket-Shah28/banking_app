...
<div class="container-fluid" style="padding-top:100px;">
  ...
  <!-- Filters: account dropdown, start/end -->
  <div class="filter-card">
    <div class="row g-3 align-items-end">
      <div class="col-md-4">
        <label for="accountNumber">Account Number</label>
        <select id="accountNumber" class="form-select">
          <option value="">-- All Accounts --</option>
          <c:forEach var="acc" items="${accountNumbers}">
      ...
      <div class="col-md-2 d-flex gap-2">
        <button id="resetBtn" class="btn-profile w-100" type="button" style="height:46px; align-self:flex-end;">Reset</button>
      </div>
    </div>
  </div>
    ...
    <table id="transactionsTable" class="table table-custom text-center align-middle w-100">
      ...
      <tbody>
        <c:forEach var="txn" items="${transactions}">
          <tr data-account="${txn.accountNumber}" data-timestamp="${txn.timestamp.time}">
            <td></td>
            <td>
              <c:choose>
  ...
  (function () {
    ...
    function filterTable() {
      ...
      rows.forEach(row => {
        ...
        if (acct && acct !== rowAccount) show = false;
    ...
    document.addEventListener('DOMContentLoaded', function () {
      document.getElementById('accountNumber').addEventListener('change', filterTable);
